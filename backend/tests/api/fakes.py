"""The fakes the §8 API and §9 SSE tests run against.

``FakeConn`` answers the handful of statements the API and the agent tools
issue, out of dicts; ``FakeArq`` records enqueued jobs. The alternative — a live
Postgres — would make the route tests an integration suite, and Phase 2 already
has one of those for the SQL itself.

These live here rather than in ``conftest.py`` because thirteen test modules
import them by name. A conftest is loaded by pytest for its fixtures; it is not
meant to be a library, and importing ``FakeConn`` from one was the tell.
"""
